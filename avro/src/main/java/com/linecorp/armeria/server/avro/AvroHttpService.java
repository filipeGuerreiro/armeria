 ...
 *
 * @see AvroSerializationFormatFactories
 */
public final class AvroHttpService extends DecoratingService<RpcRequest, RpcResponse, HttpRequest, HttpResponse>
        implements HttpService {

    private static final Logger logger = LoggerFactory.getLogger(AvroHttpService.class);

    private static final String PROTOCOL_NOT_SUPPORTED = "Specified content-type not supported";

    private static final String ACCEPT_AVRO_PROTOCOL_MUST_MATCH_CONTENT_TYPE =
            "Avro protocol specified in Accept header must match " +
    ...
    /**
     * Creates a new instance of {@link AvroHttpServiceBuilder} which can build an instance of {@link AvroHttpService}
     * fluently.
     *
     * <p>The default SerializationFormat {@link AvroSerializationFormats#BINARY} will be used when client
     * does not specify one in the request, but also supports {@link AvroSerializationFormats#values()}.
     * </p>
     *
     ...
     */
    public static AvroHttpServiceBuilder builder() {
        return new AvroHttpServiceBuilder();
    }

    /**
     * Creates a new {@link AvroHttpService} with the specified service implementation, supporting all thrift
     * protocols and defaulting to {@link AvroSerializationFormats#BINARY Binary} protocol when the client
     * doesn't specify one.
     *
     * <p>Currently, the only way to specify a serialization format is by using the HTTP session
     * protocol and setting the {@code "Content-Type"} header to the appropriate
     * {@link SerializationFormat#mediaType()}.
     *
     * @param implementation an implementation of {@code *} or {@code *.Callback} protocol interface
     *                       generated by the Apache Avro compiler
     */
    public static AvroHttpService of(Object implementation) {
        return of(implementation, AvroSerializationFormats.BINARY);
    }
    ...
    public static AvroHttpService of(Object implementation,
        ...
        return new AvroHttpService(AvroCallService.of(implementation),
                                newSupportedSerializationFormats(defaultSerializationFormat,
                                                                 AvroSerializationFormats.values()));
    }

    /**
     * Creates a new {@link AvroHttpService} with the specified service implementation, supporting only the
     ...
     */
    public static AvroHttpService ofFormats(
    ...
    /**
     * Creates a new {@link AvroHttpService} with the specified service implementation, supporting the protocols
     ...
     * {@link AvroSerializationFormats#BINARY Binary} protocol when the client doesn't specify one.
     *
     * <p>Currently, the only way to specify a serialization format is by using the HTTP session
     * protocol and setting the {@code "Content-Type"} header to the appropriate
     * {@link SerializationFormat#mediaType()}.
     */
    public static Function<? super RpcService, AvroHttpService> newDecorator() {
        return newDecorator(AvroSerializationFormats.BINARY);
    }
    ...
    public static Function<? super RpcService, AvroHttpService> newDecorator(
        ...
        return delegate -> new AvroHttpService(delegate, supportedSerializationFormatArray);
    }
    ...
    private final AvroCallService avroService;

    AvroHttpService(RpcService delegate, SerializationFormat[] supportedSerializationFormatArray) {
        super(delegate);
        avroService = findAvroService(delegate);
    ...
    private static AvroCallService findAvroService(Service<?, ?> delegate) {
        final AvroCallService thriftService = delegate.as(AvroCallService.class);
    ...
    public HttpResponse serve(ServiceRequestContext ctx, HttpRequest req) throws Exception {
        ...
        if (!validateAcceptHeaders(req, serializationFormat)) {
            return HttpResponse.of(HttpStatus.NOT_ACCEPTABLE,
                                   MediaType.PLAIN_TEXT_UTF_8, ACCEPT_AVRO_PROTOCOL_MUST_MATCH_CONTENT_TYPE);
        }
    ...
    private void decodeAndInvoke(
        ...
        try (HttpData content = req.content()) {
            ...
            final AvroServiceEntry entry = entries().get(serviceName);
