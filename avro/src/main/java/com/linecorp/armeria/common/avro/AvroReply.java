 ...
 * its header ({@link Schema}). It is exported to {@link RequestLog#responseContent()} when a Thrift call
 * is processed.
 */
@JsonSerialize(using = AvroJacksonSerializers.AvroReplyJsonSerializer.class)
public final class AvroReply extends AvroMessage {

    @Nullable
    private final GenericData result;
    @Nullable
    private final AvroRuntimeException exception;

    /**
     * Creates a new instance that contains a Thrift {@link TMessageType#REPLY} message.
     */
    public AvroReply(AvroMessage header, GenericData result) {
        super(header);
     ...
     */
    public AvroReply(AvroMessage header, AvroRuntimeException exception) {
        super(header);
     ...
     */
    public AvroRuntimeException exception() {
        if (!isException()) {
    ...
    public boolean equals(@Nullable Object o) {
        ...
        if (!(o instanceof AvroReply)) {
            return false;
        }

        final AvroReply that = (AvroReply) o;
