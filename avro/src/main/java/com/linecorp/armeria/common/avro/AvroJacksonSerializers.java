...
final class AvroJacksonSerializers extends Serializers.Base implements Serializable {

    private static final long serialVersionUID = -285900387635271875L;

    private final boolean useNamedEnums;

    AvroJacksonSerializers(boolean useNamedEnums) {
    ...
    public JsonSerializer<?> findSerializer(SerializationConfig config, JavaType type,
        ...
        if (AvroCall.class.isAssignableFrom(rawType)) {
            return new ThriftCallJsonSerializer(useNamedEnums);
        }
        if (AvroReply.class.isAssignableFrom(rawType)) {
            return new AvroReplyJsonSerializer(useNamedEnums);
        }
    ...
    private static String serializeTBaseLike(Consumer<TProtocol> writer, boolean useNamedEnums) {
        ...
        final TProtocolFactory factory = useNamedEnums ? AvroSerializationFormatFactories.TEXT_NAMED_ENUM
                                                       : AvroSerializationFormatFactories.TEXT;
    ...
    /**
     * Jackson {@link JsonSerializer} for {@link AvroCall}.
     */
    static final class ThriftCallJsonSerializer extends StdSerializer<AvroCall> {
        ...
        ThriftCallJsonSerializer(boolean useNamedEnums) {
            super(AvroCall.class);
            this.useNamedEnums = useNamedEnums;
        }

        @Override
        public void serialize(AvroCall value, JsonGenerator gen, SerializerProvider provider)
                throws IOException {
            gen.writeStartObject();
    ...
    /**
     * Jackson {@link JsonSerializer} for {@link AvroReply}.
     */
    static final class AvroReplyJsonSerializer extends StdSerializer<AvroReply> {
        private static final long serialVersionUID = -783551224966265213L;

        private final boolean useNamedEnums;

        AvroReplyJsonSerializer() {
            this(false);
        }

        AvroReplyJsonSerializer(boolean useNamedEnums) {
            super(AvroReply.class);
            this.useNamedEnums = useNamedEnums;
        }

        @Override
        public void serialize(AvroReply value, JsonGenerator gen, SerializerProvider provider)
                throws IOException {
            if (value == null) {
