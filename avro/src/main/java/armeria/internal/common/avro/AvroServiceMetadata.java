...
package armeria.internal.common.avro;
 ...
 */
public final class AvroServiceMetadata {

    private static final Logger logger = LoggerFactory.getLogger(AvroServiceMetadata.class);
     ...
     */
    private final Map<String, AvroFunction> functions = new HashMap<>();

    /**
     * Creates a new instance from a Thrift service implementation that implements one or more Thrift service
     * interfaces.
     */
    public AvroServiceMetadata(Object implementation) {
     ...
     */
    public AvroServiceMetadata(Iterable<?> implementations) {
     ...
     */
    public AvroServiceMetadata(Class<?> serviceType) {
    ...
    private void registerFunction(Class<?> iface, String name,
        ...
        try {
            final AvroFunction f;
            if (func instanceof ProcessFunction) {
                f = new AvroFunction(iface, (ProcessFunction) func, implementation);
            } else {
                f = new AvroFunction(iface, (AsyncProcessFunction) func, implementation);
            }
    ...
    /**
     * Returns the {@link AvroFunction} that provides the metadata of the specified Thrift function.
     *
     * @return the {@link AvroFunction}, or {@code null} if there's no such function.
     */
    @Nullable
    public AvroFunction function(String method) {
